...
const AnimatedButton = ({
  ...
  const variants = {
    ...
    success: `
      ...
      active:scale-95
      ${glowEffect ? 'hover:animate-pulse-once' : ''}
    `,
    danger: `
  ...
  const getAnimationClass = () => {
    ...
    switch (animationType) {
      ...
      case 'float':
        return 'hover:animate-float-particles';
      case 'all':
        return 'hover:animate-pulse hover:animate-glow';
      default:
        return '';
    }
  };
      ...
      {variant === 'legendary' && !disabled && !loading && (
        <div className="absolute inset-0 pointer-events-none">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="absolute w-1 h-1 bg-yellow-300 rounded-full opacity-70 animate-float-particles"
              style={{
                left: `${20 + i * 15}%`,
                top: `${30 + (i % 2) * 40}%`,
